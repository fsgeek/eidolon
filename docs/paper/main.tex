...
\begin{abstract}
Flexible Paxos requires only that Phase~1 and Phase~2 quorums intersect, leaving the shape of those quorums as a design choice. We show that mapping a crumbling-wall quorum construction to a physically tiered network makes the protocol's safety and liveness properties \emph{legible}: an operator can read which tiers retain global consensus capability directly from the wall structure and the current connectivity state, without runtime probing. Using a 10-node Earth/LEO/Moon/Mars topology as a magnifying glass, we confirm that a flat Phase~1 family (requiring all tiers) produces 0\% global success during conjunction blackout by construction, while a per-tier crumbling-wall family over the same topology produces 100\% at 183~ms latency---the only change is quorum geometry. This is a structural consequence of the design, not an empirical discovery; the 50-seed sweep confirms that message scheduling introduces no variance. Under the wall construction, three of four tiers retain global liveness during Mars disconnection; only the disconnected tier loses it. Consensus latency at each tier equals the speed-of-light round-trip to Earth: 131~ms from LEO, 5.1~s from Moon. A comparison between sparse and full-coverage network topologies separates the wall's quorum obligations from network reachability as independent constraints on liveness. All results are design-level; quorum intersection is verified exhaustively in TLA+. The source repository, sweep data, and reproduction commands are available at \url{https://github.com/fsgeek/eidolon}.
\end{abstract}
...
Classic Paxos~\cite{lamport1998} uses majority quorums for both phases---Phase~1 (prepare/promise) and Phase~2 (accept/accepted)---guaranteeing intersection via the pigeonhole principle.
...
The wall maps tiers to rows, ordered bottom (Earth, fast) to top (Mars, slow). We index tiers from the bottom: $T_0 = E$ (Earth), $T_1 = L$ (LEO), $T_2 = U$ (Moon), $T_3 = M$ (Mars). A proposer at tier~$i$ forms its Phase~1 quorum by reading \emph{downward} from row~$i$: it needs at least one respondent from its own tier and every tier below.
...
\]
A proposer at tier~$i$ needs at least one node from every tier at or below it ($j \le i$). For example, $\mathcal{Q}_1^{(2)}$ (Moon) requires intersection with $T_0$ (Earth), $T_1$ (LEO), and $T_2$ (Moon)---every tier from Moon down to Earth. For relaxed Phase~2 (Section~\ref{sec:relaxed}), Phase~1 additionally requires enough Earth nodes to satisfy a pigeonhole intersection constraint.
...
We verified this in TLA+, a formal specification language for concurrent systems whose model checker (TLC) exhaustively explores all reachable states to verify that safety invariants hold. The \texttt{ExhaustiveIntersection} specification checks strict $Q_2$, relaxed $Q_2$ ($k = 4$ and $k = 3$), and crash-fault constructions across all four tiers' $Q_1$ families. The \texttt{QuorumIntersection} specification explores the same property via state-space search (11{,}789 states). No intersection failure was found.

Safety is verified exhaustively over a structurally equivalent reduced topology. The reduced topology collapses each tier to a representative subset, yielding a 6-node system ($3E + 1L + 1U + 1M$). Cross-intersection requires $Q_1 \cap Q_2 \neq \emptyset$; since every $Q_1$ contains at least one node from $T_0$ (Earth) and $Q_2 \subseteq T_0$, intersection holds regardless of $|T_0|$. Safety thus depends only on tier membership, not on the number of nodes per tier, so the reduction preserves all safety properties. Liveness properties (crash tolerance, progress under partial failure) are topology-size-dependent and are evaluated only at full scale. Over this reduced topology, single-decree Paxos agreement was model-checked exhaustively (67M~states, no violation). As additional confidence, we explored 6.5 billion states of the full 10-node topology over 24 hours of continuous model checking with zero safety violations; full state-space exploration at this scale is computationally infeasible. The reduced-topology proof is the primary safety argument. Specifications are in \texttt{tla/}.
...
This is a \emph{legibility property}: given the wall structure and any connectivity state~$C$, the set of tiers retaining global Phase~1 capability is determined without runtime probing. For any tier~$i$, global Phase~1 succeeds if and only if every tier~$j \le i$ is reachable from~$i$ in~$C$---an $O(\text{tiers})$ check that requires no enumeration of quorum subsets. This is what distinguishes legibility from mere computability.
...
Per-phase timeout & 500~ms & proposer gives up \\
...
The flat construction produces 0\% during-blackout success at every sweep point---a tautology of its own design, since it requires Mars nodes that are unreachable. The crumbling wall produces 100\% at every point, because the Earth-based proposer's Phase~1 needs only Earth nodes. The zero variance across 50 seeds confirms that the outcome is structurally determined by quorum geometry and link state, not influenced by message scheduling or concurrency artifacts---a fact that was not obvious before running the sweep. We present this comparison not as an empirical finding but as a calibration: it isolates quorum geometry as the sole variable and confirms that the simulator correctly implements both constructions.
...
Let the relaxed Phase~2 family require $k$ of $|E|$ Earth nodes. We evaluate $k = 4$ (tolerates one crash) and $k = 3$ (tolerates two). Phase~1 requires correspondingly more Earth nodes to maintain intersection: if Phase~2 requires $k$-of-$|E|$ Earth nodes, then any two Phase~2 quorums overlap in at least $|E| - k + 1$ nodes (by pigeonhole). Phase~1 must contain at least $|E| - k + 1$ Earth nodes to guarantee intersection with every Phase~2 quorum. For $k = 4$ out of 5 Earth nodes, this means at least 2 Earth nodes in every $Q_1$; for $k = 3$, at least 3. Both constructions were verified exhaustively in TLA+.
...
\textbf{The weakest link migrates.} At two crashes with standard Earth-local quorums, the \emph{global} construction (3-of-5, spanning all tiers) maintains 98\% during-blackout success---but the \emph{Earth-local} construction drops to 49.7\% because it cannot form a Phase~1 quorum of 4 with only 3 remaining Earth nodes (rows~5--6). The global construction's crash-tolerance advantage is arithmetic: the global quorum draws from 10 nodes rather than 5, permitting more failures before quorum loss. The structural insight is not the count but the legibility---which crashes matter depends on their tier, not just their number.
...
\textbf{Topology-scoped consistency.} During blackout, each tier maintains local consistency; cross-tier state becomes stale in a structured way determined by the wall. The wall makes the consistency topology explicit, allowing operators to reason about failure modes from the quorum design rather than discovering them at runtime.

\textbf{Leadership hierarchy.} The wall imposes a cost gradient on Multi-Paxos leader election that symmetric quorum constructions cannot express. The exhaustive TLA+ enumeration quantifies this directly:

\begin{center}
\small
\begin{tabular}{lrr}
\toprule
Initiating tier & Strict $|\mathcal{Q}_1^{(i)}|$ & Relaxed $|\mathcal{Q}_1^{(i)}|$ ($k{=}4$) \\
\midrule
Earth (tier 0, bottom) & 992 & 832 \\
LEO (tier 1) & 496 & --- \\
Moon (tier 2) & 248 & --- \\
Mars (tier 3, top) & 217 & 182 \\
\bottomrule
\end{tabular}
\end{center}

Earth has 4.6$\times$ more valid Phase~1 quorums than Mars. In Multi-Paxos, where leader election \emph{is} Phase~1, this means Earth leader election is not only faster (sub-second vs.\ minutes) but more robust: there are more ways to form the quorum, so more crash patterns are survivable. The gradient is monotonic---leadership flexibility increases from top to bottom of the wall.

WPaxos~\cite{ailijiang2019wpaxos} uses Flexible Paxos with symmetric grid quorum layouts, where $Q_1$ spans $Z - f_z$ zones regardless of which node initiates. The grid's intersection proof requires uniform $Q_1$ size; there is no ``who is asking'' parameter. The wall's $\mathcal{Q}_1^{(i)}$ is a different mathematical object: a family of families, indexed by tier. This means partition failover cost is topology-proportional: if the leader is on Earth and Mars goes dark, the disconnection is a non-event for leader election. Safety follows from the Phase~2 anchor: since $\mathcal{Q}_2 \subseteq E$, all accepted values reside on Earth nodes, so Earth-scope Phase~1 discovers all relevant state.

\textbf{Toward tiered write authority.} Mars writes currently require routing to an Earth proposer ($\sim$12-minute round trip). A natural extension is \emph{scoped write leases}: the Earth leader grants Mars write authority over a slot range using a Mars-local Phase~2 quorum. On blackout, the lease expires; Earth reclaims authority with Earth-scope Phase~1, and divergent writes reconcile via conflict-resolution semantics (MRDTs/CRDTs). This would introduce per-tier durability characteristics, making consistency guarantees a function of wall position. We leave formal construction of tiered leases to future work.


\section{Conclusion}

Mapping a crumbling-wall quorum construction to a physically tiered network produces a consensus protocol whose failure modes are legible from the geometry. The wall crumbles from the top: during disconnection, only the unreachable tier loses global liveness. Three of four tiers retain global consensus during hard Mars blackout, at latencies determined by the speed of light to Earth.

Beyond legibility, the wall encodes a leadership hierarchy: because per-tier Phase~1 scope varies with wall position, leader election cost is topology-proportional---a property that symmetric grid quorums cannot express. The interplanetary setting is the magnifying glass; the underlying principle---that quorum geometry should mirror the topology it serves---applies to any system with tiered latency asymmetry and structured disconnection.


\appendix
\section{Claim-to-Artifact Traceability}
\label{app:traceability}
Table~\ref{tab:traceability} maps the paper's main claims to executable artifacts. Paths are relative to the repository root. Results were generated with Python~$\ge$3.14 and SimPy~$\ge$4.1.1. Commit hash at submission: \texttt{[TO BE PINNED]}.
