...
\item \textbf{Legible liveness degradation.} Under Mars conjunction blackout, the wall crumbles from the top: only Mars loses global Phase~1 capability. Earth, LEO, and Moon retain it. Consensus latency at each tier equals the speed-of-light round-trip to Earth: 183~ms (Earth), 131~ms (LEO), 5.1~s (Moon). As baseline calibration, a flat construction over the same topology produces 0\% global success during blackout---a structural guarantee of both constructions, confirmed by the simulator across all 18 sweep points with zero variance (Section~\ref{sec:results}).
...
We verified this in TLA+, a formal specification language for concurrent systems. Its model checker, TLC, checks safety by exhaustively exploring every reachable state---every possible interleaving of messages and failures---and verifying that a safety invariant holds in each one. When TLC completes without finding a violation, the property is proven for all behaviors of the model. The \texttt{ExhaustiveIntersection} specification checks strict $Q_2$, relaxed $Q_2$ ($k = 4$ and $k = 3$), and crash-fault constructions across all four tiers' $Q_1$ families (11{,}789 states, complete). No intersection failure was found.
...
The flat construction produces 0\% during-blackout success at every sweep point, which is a tautology of its own design, since it requires Mars nodes that are unreachable. The crumbling wall produces 100\% at every point, because the Earth-based proposer's Phase~1 needs only Earth nodes. The zero variance across 50 seeds confirms that the outcome is structurally determined by quorum geometry and link state, not influenced by message scheduling or concurrency artifacts, a fact that was not obvious before running the sweep. We present this comparison not as an empirical finding but as a calibration: it isolates quorum geometry as the sole variable and confirms that the simulator correctly implements both constructions.
...
Flat quorum constructions conflate inter-tier obligation with intra-tier replication. The crumbling-wall construction separates them: one witness per tier on the downward path for cross-tier safety, with each tier's internal replication invisible to the wall. From this separation follow legibility ($O(\text{tiers})$ liveness determination without probing), a leadership cost gradient that symmetric constructions cannot express, and composable fault tolerance where each tier's durability decisions are independent of the wall's quorum geometry.
