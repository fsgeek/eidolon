...
The structural state itself has no intrinsic valence. Section~\ref{sec:valence} reports what happened in the registered single-decree experiment under its stated topology, retry policy, and retry budget. That experiment measures one consequence of the residual state; it does not turn the state into a universal label.
...
\centering
\small
\caption{Per-tier global consensus during Mars conjunction blackout (full-coverage topology, hard blackout, 186~s Mars delay, 1800~s blackout, 50 seeds). Rates are identical across seeds; latency and recovery are means, with 95\% CIs reported in the text.}
\label{tab:pertier}
\begin{tabular}{llrrr}
\toprule
...
\centering
\small
\caption{Per-tier global consensus under sparse topology (hard blackout, 186~s Mars delay, 900~s blackout, 50 seeds). All rates are identical across seeds; structural outcomes are reported without CIs.}
\label{tab:sparse}
\begin{tabular}{llr}
\toprule
