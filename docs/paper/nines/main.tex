...
This is a \emph{legibility property}: given the wall structure and a tier-level summary of the connectivity state~$C$, the set of tiers retaining global Phase~1 capability is determined without trial quorum formation. For any tier~$i$, global Phase~1 succeeds if and only if every tier~$j \le i$ is reachable from~$i$ in~$C$, which is an $O(\text{tiers})$ check that requires no enumeration of quorum subsets. This is what distinguishes legibility from mere computability. Global \emph{operation}, as opposed to Phase~1 recovery, additionally requires the Phase~2 quorum to be reachable; Section~\ref{sec:reachability} shows the two conditions coming apart.
...
\midrule
Earth & bottom (0) & 100.0\% $\pm$ 0.0 & 0.183~s & 62.6~s \\
LEO & tier 1 & 100.0\% $\pm$ 0.0 & 0.131~s & 61.8~s \\
Moon & tier 2 & 100.0\% $\pm$ 0.0 & 5.131~s & 6.7~s \\
Mars & top (3) & 0.0\% $\pm$ 0.0 & --- & --- \\
\bottomrule
\end{tabular}
\end{table}

Three of four tiers retain global consensus capability during hard Mars blackout. Only Mars, which is the disconnected tier, loses it. The liveness failure crumbles from the top of the wall, exactly as the construction predicts. (Latency and recovery-lag CIs are $\pm 0.000$~s and $\pm 0.01$~s respectively.)
...
\begin{tabular}{lrrrl}
\toprule
Regime & \makebox[0pt][r]{P1 min (E/M)} & P2 & Tol. & Commit \\
\midrule
...
\textbf{Topology-scoped consistency.} During blackout, the wall separates tiers by which \emph{capabilities} they retain. Earth, LEO, and Moon can still form both global quorums: they can complete Phase~1 (discover every committed value---any Phase~1 quorum intersects every Phase~2 quorum, so nothing committed can be missed) and Phase~2 (extend the global log). Under the standard Paxos client discipline, their operations on the global log remain linearizable. Mars retains neither capability: there are Earth-committed values Mars cannot discover until connectivity resumes, so it is confined to reading its local replica---a stale but locally ordered prefix of the global history---and to its separate Mars-local consensus scope. The two capabilities are independent, and both matter: under the sparse topology of Section~\ref{sec:reachability}, LEO can satisfy its Phase~1 obligation and therefore still \emph{learn} the global history, but cannot reach the strict Phase~2 quorum and so cannot extend it. The wall determines capabilities; precisely which consistency contract to expose over a degraded capability set---and how the local scope reconciles on reconnection---is a service-level decision the wall does not make. What it makes legible is the capability boundary itself, and hence when that decision is being made: refusing operations at a cut-off tier preserves consistency by sacrificing availability, while continuing to serve locally is an explicit adoption of a weaker, local contract. The degradation is epistemic before it is electoral: what a cut-off tier loses first is not a vote but the ability to \emph{discover} what has been committed---the wall bounds what each tier can currently know. At interplanetary distances, the gap between the global and local regimes is twenty-two minutes wide---the distinction that distributed systems students struggle with as an abstract definition becomes a physical fact you could set a timer by.
...
Crumbling wall: 100\% during blackout across all 18 sweep points, 183~ms latency. & All rows show 100\% during-blackout. & \texttt{results/step9\_crumbling/\allowbreak step9\_sweep.csv}; \texttt{experiments/\allowbreak step9\_sweep.py}. \\
...
Paxos agreement model-checked. & 67M states (reduced, complete), 6.5B states (full, partial). & \texttt{tla/\allowbreak PaxosSmall.tla}; \texttt{tla/\allowbreak PaxosFull.tla}. \\
\bottomrule
\end{tabular}
\end{table*}
