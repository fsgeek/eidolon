...
\begin{abstract}
One reachable-node count once answered acquisition and commit together in familiar majority-based Paxos because phase symmetry made the two formability predicates coincide. Flexible Paxos preserves cross-phase safety while allowing them to separate. We show that containment exactly characterizes both mixed states: acquisition without commit is impossible if and only if every Phase~1 quorum contains a Phase~2 quorum, and commit without acquisition is impossible under the mirrored containment. In a pre-registered single-decree experiment with a fixed eight-round healthy-proposer budget and the modeled retry policy, the results reversed the predicted behavioral ordering. An acquisition-only incumbent matched a healthy contender on every recorded metric and injected an accepted value that later acquisition then preserved, whereas at an eight-round incumbent budget a commit-capable but acquisition-incapable incumbent prevented the healthy proposer from deciding in all 50 seeds. We apply the characterization to a wall-shaped quorum construction over a 5/1/1/3 Earth/LEO/Moon/Mars topology. At Phase~2 threshold $k=3$, the $(1,0)$ gap is absent at every tier. Both gaps are absent for Earth under both connectivity readings and for LEO under the self-reachable reading; Moon and Mars retain $(0,1)$ exposure. The wall keeps Phase~2 on Earth and yields an $O(\text{tiers})$ readout from supplied connectivity; it reports structural capability, not current authority. Proofs, exhaustive audit, and simulation establish design-level results for the 5/1/1/3 topology. The mapping to edge-shaped systems remains structural.
\end{abstract}
...
We approach that mismatch from the failure backwards: \emph{What did recovery need to know, what evidence did the control plane possess, and what capability did that evidence fail to establish?} GitHub's account does not describe the phase-formability problem studied in this paper, and our model does not explain the incident. It exposes the methodological conundrum. Component health, protocol success, current authority, and service capability are different answers, even where familiar system designs make them look like one scalar state.
...
The uniform case shows the smallest cost of retaining the coincidence. Subject to $q_1+q_2 \ge n+1$, a cost-minimal split can be symmetric when $n$ is odd. When $n$ is even, symmetry adds one additional participant across the two phase thresholds. This is a post-hoc structural observation, not an explanation for deployment practice or for the odd-cluster convention.
...
The prediction posed at the end of the previous section failed. It failed in reverse, just as the dated preregistration permits us to say without reconstructing the surprise. At every incumbent budget, the $(1,0)$ arm matched the healthy $(1,1)$ contender on every recorded metric: completion probability 1.000, median time to first commit 3.420~s, two healthy rounds, and seven NACKs. The incumbent completed from one to eight Phase~1 quorums as its budget increased, while the healthy outcome stayed flat. This is an experimental equivalence on the recorded metrics under these conditions, not a universal claim about $(1,0)$.
...
At $k=3$, the $(1,0)$ gap is absent at every tier. At the same threshold, both gaps are absent for Earth under both connectivity readings, and both gaps are absent for LEO under the self-reachable reading. The commit path remains on Earth. The construction therefore restores the old coincidence at named tiers without returning inter-tier latency to Phase~2. The result has a boundary: $(0,1)$ remains reachable for Moon and Mars because their Phase~1 families retain downward participation obligations.
...
The witnesses implement participation policy; legibility makes the resulting obligation readable. When a capability gap remains possible, state-aware mitigation first requires distinguishing the state. The readout supplies that predicate for connectivity data supplied to it. Enumeration and auditor artifacts are in \texttt{results/capability/} (see \autoref{app:traceability}).

\section{Reading the Wall}
\label{sec:reading}
\label{sec:expdesign}
\label{sec:results}

\subsection{Readout Interface}
\label{sec:liveness-wall}

The wall-specific readout takes named tiers, a Phase~2 threshold, an initiating tier, a reachable-node summary, and the provenance of configuration and connectivity. It returns $(R_1,R_2)$, quorum witnesses, and typed failed obligations in one pass over the tiers. It reports runtime authority as unknown and service policy as not inferred. Connectivity is an input to the interface; the readout does not detect failures, identify a current leader, establish fresh runtime authority, choose a client contract or recovery policy, or ensure that anyone acts on its output.

For tier~$i$, the pass checks whether at least one node is reachable in every required row $T_j$ with $j \le i$, then checks the Earth threshold for Phase~2. During the modeled Mars conjunction blackout, Earth, LEO, and Moon retain the rows they require. Mars cannot reach the inner rows, so its Phase~1 obligation fails (Figure~\ref{fig:blackout}). This is the promised $O(\text{tiers})$ check. It establishes structural formability for a sole, timely proposer; concurrent promises, current authority, and protocol execution remain separate.
...
\end{tikzpicture}
\caption{Structural readout under the supplied hard-blackout connectivity state in the 5/1/1/3 wall; no latency, timeout, or stochastic trial is involved. Earth, LEO, and Moon retain their required rows. Mars does not.}
\label{fig:blackout}
\end{figure}

The repository supplies planetary and edge JSON inputs to the same deterministic interface. They are a demonstration artifact, not empirical evidence. The edge input is a structural mapping and not an evaluated result: it supports no claim about terrestrial latency, availability, or prevalence. The executable interface is \texttt{experiments/\allowbreak capability\_readout.py}; input and output examples are listed in \autoref{app:traceability}.
...
The wall gives designers a usable partial answer. At $k=3$, it removes $(1,0)$ at every tier, removes both gaps for Earth under both readings and for self-reachable LEO, and keeps Phase~2 on Earth. Moon and Mars still admit $(0,1)$ because their acquisition families retain downward participation obligations. The remaining question is precise: can a structured quorum design extend acquisition/commit coincidence farther up the topology without returning inter-tier latency to the Phase~2 hot path?

Three candidates follow from the boundary, and none is a result of this paper: scoped authority could let an upper tier continue over an explicitly bounded slot or time range; gap-aware proposer behavior could use the two-bit readout to choose retry and failover behavior suited to the observed state; and multi-anchor families could reduce dependence on one anchor tier. Each changes a different part of the system and requires separate work on safety, performance, and operational behavior. The terrestrial edge mapping supplies a concrete next experiment, not evidence that any candidate works.
...
A single reachable-node count once worked because phase symmetry made the two quorum families equal and a threshold family made their shared predicate count-readable. Flexible Paxos preserves cross-phase safety after the predicates separate. Containment then gives the exact consequence: ordered predicates admit one gap direction; incomparable predicates admit both.

The wall turns that characterization into a design tool. At $k=3$, $(1,0)$ is absent at every tier. Both gaps are absent for Earth under both connectivity readings and for LEO under the self-reachable reading. Moon and Mars retain $(0,1)$ because of their downward participation obligations. No tested threshold closes both gaps everywhere, but the readout makes each remaining obligation explicit without moving Phase~2 off Earth.

The portable diagnostic is two questions: Can this system acquire authority? Can it commit? A phase-asymmetric quorum system should expose acquisition capability and commit capability separately. It should report current authority separately from both. Node health and one replica count may answer neither. Once the answers are visible, designers can combine structural capability with current authority and an explicit service contract. The open problem is how far those answers can be made to coincide without returning inter-tier latency to Phase~2.
