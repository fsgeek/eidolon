...
At 22:52 UTC on October 21, 2018, routine work to replace failing 100G optical equipment interrupted connectivity between two parts of GitHub's US East Coast infrastructure. Connectivity returned 43 seconds later. During that interval, a surviving quorum of Orchestrator's Raft group initiated database failover, exactly as configured. The old primary site retained writes that had not reached the new one; after connectivity returned, the new primaries accepted further writes; and the application tier could not tolerate the latency of the resulting cross-country topology. Recovery and degraded service lasted 24 hours and 11 minutes~\cite{warner2018github}. Consensus answered the question it was asked: a quorum of reachable replicas agreed to fail over. The questions the operators needed answered were different: what can the service still do, and what will this action cost it? This paper is about the gap between the question consensus answers and the questions operators need answered.
...
A five-node example makes the failure concrete. Configure Flexible Paxos so that commits require any two nodes and elections require four; safety holds because $2+4>5$. Two nodes fail. Writes continue: three survivors exceed the commit threshold, and the dashboard shows a majority up. But the system has silently lost the ability to replace its leader, and that loss is discovered only when the leader fails, because elections run only when something is already wrong. The missing capability is always demanded in the middle of another emergency. The postmortem will blame the leader's crash; the system became unrecoverable when the second node died, hours earlier, at a moment no monitored quantity changed. A leader-liveness monitor does not close the gap: it reports the incumbent healthy until the crash, and says nothing about whether a successor is possible.
...
The manifestation conditions are exact in the uniform case. With thresholds $q_1 > q_2$ over $n$ nodes, the two capabilities carry different failure budgets: commit formability survives $n - q_2$ failures, acquisition formability only $n - q_1$. Every reachable count in the band $[q_2, q_1)$ is a mixed state, commit formable and acquisition not, and the band's width is the threshold gap: exactly the margin the configuration was chosen to exploit, so exposure grows with the performance benefit. Discovery is deferred by structure, not chance: acquisition is exercised only when authority must change, so its loss surfaces during the recovery that demands it rather than at the failure that caused it. Reversing the threshold order reverses the band's contents: $q_1 < q_2$ yields acquisition-without-commit states instead, whose onset is visible immediately because commits fail in front of clients. Which mixed state a configuration can suffer, and across how wide a band, is therefore decided wherever the two thresholds are set. A uniform configuration admits at most one of the two mixed states; admitting both at once requires shaped families, where different failure patterns open different gaps for different proposers. That is where two comparisons stop sufficing and the general characterization of the next section becomes necessary.

For a Moon proposer under the strict wall used below (the construction of Section~\ref{sec:construction}), two reachable sets of five nodes give opposite answers; $R_1$ asks whether the reachable set supports acquisition, $R_2$ whether it supports commit:
...
The pair $(R_1,R_2)$ has four joint states. Two are familiar: $(1,1)$ makes both phases formable, and $(0,0)$ makes neither formable. We call the mixed states \emph{capability gaps}: $(1,0)$ permits acquisition but not commit formation, while $(0,1)$ permits commit formation but not acquisition. A single family used for both phases makes both gaps empty.

Operationally, $(1,0)$ has a familiar name: read-only mode. Its onset is loud, because commits fail in front of clients, and Section~\ref{sec:valence} shows it is benign under contention, because acquisition, the capability that recovers work, survives. $(0,1)$ has no established operational name: writes continue under an incumbent that cannot be replaced, and a state without a name is one that operators do not monitor for. We call it write-only under old management. Phase-capability coincidence is the semantic equality $\operatorname{Form}(\mathcal{Q}_1)=\operatorname{Form}(\mathcal{Q}_2)$; it can also hold for syntactically different families.
...
Five deviations remain recorded with their reasons in the artifact: the flip occurs before Phase~2 fan-out; the intended $(0,0)$ control is realized as $(0,1)$; the incumbent is Moon-tier; the late-$(0,1)$ arm was added to test the timing prediction; and the registered treatment exists only at $k=5$. At $k=3$, the boundary result (Proposition~\ref{prop:boundary}) makes $(1,0)$ unreachable at every tier, so there is no corresponding treatment state to rerun. That result and the wall enumeration stand independently of this experiment. These runs measure behavior inside constructed mixed states; they do not validate either structural result. How can a designer or operator read which state the topology permits?
...
\end{tikzpicture}
\caption{The crumbling-wall construction over the 5/1/1/3 topology. Each tier's Phase~1 quorum reads downward through the wall (dashed arrows). Phase~2 uses only the Earth row (shaded). Row widths vary; wider rows offer more quorum choices.}
\label{fig:wall}
\end{figure}
...
A compact framing for that question: the authority structure of reconfigurable Multi-Paxos is a hierarchical clock, $\langle$configuration, leadership, proposal$\rangle$, each counter meaningful only inside the frame above it, kept totally ordered by the protocol's hardest machinery. A reconfiguration is a tick of the slowest counter, and the open problem is what evidence must cross that tick so that two frames never run concurrently, each believing itself current. The introduction's incident shows the cost of such a fork in a system with no merge function; characterizing the evidence that prevents it is where this work points next.
...
Fixed parameters across all experiments are listed in Table~\ref{tab:params} (Appendix~\ref{app:params}). All seeds are 40--89 (50 runs per point). Results are reported as means with 95\% confidence intervals.
