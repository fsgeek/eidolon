...
\begin{abstract}
Operators often ask whether enough replicas are reachable. In familiar phase-symmetric threshold Paxos, that smaller question happened to settle two larger ones: can a proposer acquire authority, and can an authorized proposer commit? The two capabilities had the same answer, a property we call \emph{phase-capability coincidence}. Flexible Paxos preserves cross-phase safety while allowing the answers to separate. We show that containment characterizes the separation exactly: acquisition without commit is impossible precisely when every Phase~1 quorum contains a Phase~2 quorum, with the mirrored condition excluding commit without acquisition. In a pre-registered single-decree contention experiment under the modeled retry policy and fixed round budgets, the results reversed our prediction: the acquisition-only arm matched the healthy arm on every recorded metric, while a commit-capable but acquisition-incapable incumbent exhausted the healthy proposer's budget. A wall-shaped quorum fixture demonstrates both prevention and detection: containment excludes selected mixed states, and an $O(\text{tiers})$ readout names residual capability and failed obligations from supplied connectivity while leaving runtime authority and service policy unknown. Mars and LEO make prolonged and time-varying reachability visible; changing membership remains outside this fixed-epoch result.
\end{abstract}
