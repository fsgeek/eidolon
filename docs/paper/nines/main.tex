...
\begin{abstract}
A quorum control plane can answer its own question correctly without establishing that the surrounding service can perform the action recovery requires. Familiar phase-symmetric threshold Paxos obscured this distinction: acquisition and commit formability were equal, and one reachable-replica count exposed their common answer. We call that equality \emph{phase-capability coincidence}. Flexible Paxos preserves cross-phase safety while allowing the two capabilities to separate. We show that containment characterizes the separation exactly: acquisition without commit is impossible precisely when every Phase~1 quorum contains a Phase~2 quorum, with the mirrored condition excluding commit without acquisition. A pre-registered single-decree contention experiment then reversed our predicted behavioral ordering under its stated retry policy and eight-round healthy-proposer budget: the acquisition-only arm matched the healthy arm on every recorded metric, while a commit-capable but acquisition-incapable incumbent exhausted the healthy proposer's budget in all 50 seeds at the stated incumbent budget. A wall-shaped quorum fixture demonstrates both prevention and detection: containment excludes selected mixed states, and an $O(\text{tiers})$ readout names residual capability and failed obligations from supplied connectivity while leaving runtime authority and service policy unknown. Mars and LEO make prolonged and time-varying reachability visible; changing membership remains outside this fixed-epoch result.
\end{abstract}
...
The uniform case shows the smallest cost of retaining the coincidence. Subject to $q_1+q_2 \ge n+1$, a cost-minimal split can be symmetric when $n$ is odd. When $n$ is even, symmetry adds one additional participant across the two phase thresholds. This is a post-hoc structural observation, not an explanation for deployment practice or for the odd-cluster convention. Once the families differ, which mixed states must appear, and which can quorum design exclude?
...
The structure is safe; which capability gaps does it prevent?
