...
\textbf{Scope, and why single-decree is the severe case.} Under multi-Paxos a $(0,1)$ tier continues committing for as long as its authority survives, so the exposure is deferred to the moment authority must be re-acquired. Under single-decree every decree requires Phase~1, so the same state blocks every new decree immediately. The measured setting is therefore the more severe one, not a weaker proxy for it.
...
A single quorum family used for both phases supplies two guarantees, and the literature named one. Intersection is a witness to the past; coincidence---the two phase predicates agreeing in every connectivity state---is what made counting reachable nodes an adequate diagnostic, and flexible quorums spend it. We give the exact criterion for which capability gap that opens, and applying it to our own wall returns no comfortable answer: no threshold setting closes both gaps, and the harmful one persists at every tier separated from the anchor by an intervening row. Its witness obligations buy topology-legible authority and manufacture that exposure in the same stroke.

Which is why we take legibility as the result rather than the consolation. The state is invisible to the instrument operators use---every node healthy, nothing concealed, and for Multi-Paxos deployments the standard remediation is the one that makes it worse. The connectivity data is already in hand; the predicate that reads it was missing. A 10-node interplanetary topology makes that undeniable, but the gap is a property of quorum geometry meeting structured topology, and elsewhere only generous timeouts hide it.
