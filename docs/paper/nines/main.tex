...
The $k=3$ wall restores the old coincidence in several precise places without moving Phase~2 off Earth. The $(1,0)$ gap is absent at every tier. Both gaps are absent for Earth under the unconstrained and self-reachable connectivity readings, and for LEO under the self-reachable reading. Moon and Mars retain $(0,1)$ exposure. The wall is therefore a useful but bounded construction: its geometry lets a designer choose some places where the two phase questions coincide, while its witness obligations make the remaining boundary visible.
...
At $k=3$, the Earth floor contains a Phase~2 quorum, so $(1,0)$ is absent at every tier. Under the self-reachable reading, $(0,1)$ is also absent for Earth and LEO. Moon and Mars retain it because each has an intervening witness row between itself and the anchor; no value of $k$ removes that obligation. The endpoints explain why $k=3$ is the useful middle: at $k \le 2$ every tier carries $(0,1)$ exposure, while at $k \ge 4$ Proposition~\ref{prop:boundary}'s bound is violated and $(1,0)$ opens everywhere. No tested value of $k$ closes both states at every tier.
