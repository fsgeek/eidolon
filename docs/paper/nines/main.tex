...
That argument is frequency arbitrage: Phase~2 runs on every command and Phase~1 only at elections, so move cost onto the rare path. Its premise is that \emph{rare} implies \emph{independent}---that the expensive path's price can be raised because it is seldom paid. Elections, however, are demanded precisely when connectivity has degraded. The demand for Phase~1 is correlated with its price, and the correlation is adversarial: the configuration is cheapest in exactly the states where it is not needed.

Behind that lies something the arbitrage argument does not name. A single quorum family used for both phases provides two distinct guarantees, and the literature refined only one of them. The first is \emph{intersection}, a witness to the past: every electorate contains a node that remembers every committed decree. This is the requirement Flexible Paxos isolated, and forty years of coterie theory sharpened it. The second is what we will call \emph{coincidence}, a witness to the future: when the two phase families are identical, ``can elect'' and ``can commit'' are the same predicate over connectivity states, so a single number---how many nodes are reachable---answers both questions at once. This is why counting healthy nodes was ever an adequate operational diagnostic.

Under a uniform threshold both guarantees fall out of the same arithmetic, so they were never observationally distinct and coincidence was never named. It is worth being precise about what supplies it: coincidence requires phase \emph{symmetry}, not majority specifically. Any self-intersecting coterie used for both phases yields it. Majority is simply the availability-optimal and load-suboptimal~\cite{peleg1995} way to obtain both witnesses at once.

Flexibility is therefore, definitionally, the act of spending coincidence. Once the families differ, ``can elect'' and ``can commit'' come apart, and the \emph{direction} of the asymmetry selects which way. Making elections at least as hard as commits admits only states where a proposer can elect but not commit; making commits at least as hard admits only the converse. These two are not equivalent in consequence, and we measure both (Section~\ref{sec:results}): the first is a self-throttling value injector that terminates by helping, matching a constructive reading already present in the Flexible Paxos paper; the second is fast-failing, cheap to retry, aimed at the shared anchor, and structurally unable to end the contention it creates. The standard recommendation---keep the commit quorum small so the hot path stays fast---makes elections the harder phase, and so admits exclusively the second class. The field chose its direction of asymmetry on a cost-per-execution argument that never priced consequence-per-state.

But how should that design space be explored? Most Flexible Paxos deployments choose quorum sizes, a number for each phase, without considering the \emph{shape} of those quorums. Peleg and Wool~\cite{peleg1995} showed that quorum geometry can be engineered around combinatorial structure rather than simple counts, with crumbling-wall constructions (nodes arranged in rows of varying width; a quorum takes one complete row together with a representative from every row below it, and the wall ``crumbles'' as rows lose members to failure) that achieve high availability through asymmetric quorum families. This paper composes these two ideas: Flexible Paxos's intersection requirement with Peleg and Wool's wall construction, mapped onto a physically tiered network where the rows of the wall correspond to latency tiers. The composition itself was suggested by Howard et al.; our contribution is what the composition is shown to yield---per-tier liveness characterization under scheduled disconnection, legibility extended from liveness to per-tier consistency semantics, and a leadership cost gradient.
...
Separation, though, does not buy coincidence back. On a tiered topology phase symmetry is expensive away from the anchor, because every tier the commit family spans is paid for on every command---which is the entire reason the commit path was anchored to the fast tier in the first place. We give the exact criterion for when each capability gap arises (Section~\ref{sec:capability}), and applying it to our own construction returns an uncomfortable result: \emph{no} setting of the Phase~2 threshold closes both gaps. Relaxing it far enough to close one opens the other, and at the best available setting the commit-capable-but-election-incapable state---the one our measurements identify as the harmful direction---persists at every tier separated from the anchor by an intervening row. It is not an artifact of parameter choice; it follows from those tiers carrying witness obligations, and no threshold removes an obligation.

We take this as the paper's central point rather than as a limitation to be confessed later. The state is not detectable by the instrument operators actually use: every node is healthy, nothing is concealed, and no node-health summary reports that elections have become impossible while commits continue to succeed. The connectivity data required is already in hand; what was missing is the predicate that reads it. When a capability gap cannot be closed by construction, making it legible is not a consolation---it is the only mitigation left.
...
\begin{enumerate}
\item \textbf{Capability states, with exact criteria.} Election and commit formability are two predicates over the connectivity state, admitting four joint states. We characterize both mixed states by quorum containment---one is unreachable exactly when every election quorum contains a commit quorum, the other under the mirrored condition---give the closed-form threshold boundary for our construction, and show the criterion collapses to a comparison of two integers for uniform families. Exhaustive enumeration establishes that no threshold setting closes both states on our construction, and that the residual exposure sits at the tiers carrying witness obligations (Section~\ref{sec:capability}).
