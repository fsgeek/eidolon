...
Under Multi-Paxos, a still-authorized $(0,1)$ leader can continue committing and encounter the missing Phase~1 capability only when authority must be reacquired. Single-decree Paxos pays Phase~1 for every decree, so this experiment exposes the state immediately. The operational response can therefore reverse with the state: restarting an authorized $(0,1)$ leader can discard the only currently usable authority, while acquisition through a $(1,0)$ state can recover an accepted value. A node-health summary distinguishes neither case.
