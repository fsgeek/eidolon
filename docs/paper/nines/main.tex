...
\item \textbf{Legible degradation of liveness and consistency.} Under Mars conjunction blackout, the wall crumbles from the top: only Mars loses global Phase~1 capability. Earth, LEO, and Moon retain it. Consensus latency at each tier equals the speed-of-light round-trip to Earth: 183~ms (Earth), 131~ms (LEO), 5.1~s (Moon). The same structure determines semantics: a tier retains global linearizability exactly when it can reach every tier below it, and degrades to local sequential consistency otherwise (Section~\ref{sec:discussion}). Section~\ref{sec:results} includes a flat-vs-wall calibration on the same topology.
...
\begin{tabular}{lrrrl}
\toprule
Regime & P1 min & P2 & Tol. & Commit cost \\
\midrule
...
\textbf{Topology-scoped consistency.} During blackout, the wall partitions the system into tiers that are linearizable with respect to the global log and tiers that retain only sequential consistency locally. Earth, LEO, and Moon can still complete global Phase~1, discover all committed values, and write new ones that are ordered with respect to the full history---they remain linearizable. Mars nodes can still read their local replicas and those reads respect the order of locally observed writes, but there are Earth-committed values that Mars cannot discover until connectivity resumes. Mars is sequentially consistent but not linearizable with respect to the global log. The wall tells you exactly which regime each tier is in: a tier at position~$i$ is globally linearizable if and only if it can reach every tier $j \le i$; otherwise its consistency degrades to local sequential consistency in a way the wall makes explicit. The degradation is epistemic, not electoral: what Mars loses is not a vote but the ability to \emph{discover} what has been committed---the wall bounds what each tier can currently know. At interplanetary distances, the gap between the two regimes is twenty-two minutes wide---the distinction that distributed systems students struggle with as an abstract definition becomes a physical fact you could set a timer by.
